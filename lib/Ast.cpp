...
CondExpression::CondExpression(std::unique_ptr<Token>&& t,
							   std::unique_ptr<Expression>&& a):
	Expression(std::move(t)), args(std::move(a))
{
}

std::string CondExpression::show()
{
	return "(cond " + args->show() + ")";
}

void CondExpression::accept(xerxzema::AstVisitor &v)
{
	v.visit(this);
}
...
void AstVisitor::visit(CondExpression *e)
{
	handle_default(e);
}
